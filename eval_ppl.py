...
from caches import (
   DynamicKV, PiToMeKV, StreamKV, STREAM, PITOME, BASELINE
)
import wandb 


def get_cache_class(args):
   if args.experiment == STREAM:
      return StreamKV(args.window_size, args.attention_sink_size)
   elif args.experiment == PITOME:
      return PiToMeKV(args.window_size, args.attention_sink_size) 
   else:
      return DynamicKV()
...
def compute_perplexity(
   ...
   past_key_values = get_cache_class(args=args)
   ...
   for text in itertools.islice(dataset, num_samples):
      ...
      for idx in pbar:
         ...
         wandb.log({
            ...
            "ppl": perplexity.item(), 
            "overall_ppl": torch.tensor(logs["nll"]).mean().exp().item(), 
            "cuda_vram_allocated": torch.cuda.memory_allocated(0) / 1024 / 1024 / 1024, 
...
def main():
   ...
   parser.add_argument(
      "--experiment", choices=[PITOME, BASELINE, STREAM], default="transformers"
   )
