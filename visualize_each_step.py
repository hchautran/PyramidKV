...
if __name__ == '__main__':
   ...
   for sigma in tqdm([3.0, 4.0, 5.0, 6.0]):
      ...
      for layer_idx, attentions in enumerate(all_layers_attentions):
         ...
         key_correlation = df['attention layer'].corr(df['key energy'])
         value_correlation = df['attention layer'].corr(df['value energy'])
         key_mean_correlation = df['attention layer'].corr(df['key mean energy'])
         # attention_correlation= df['attention'].corr(df['attention layer'])
         print(key_correlation, value_correlation, key_mean_correlation )
         wandb.log({
            'layer idx':layer_idx,
         ...
         plot_attention_energy_map(attention * 10000, energy_key)
